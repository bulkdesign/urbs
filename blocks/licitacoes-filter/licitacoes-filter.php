...
$post_types = get_field( 'post_types' ) ? get_field( 'post_types' ) : apply_filters( 'licitacoes_filter_post_types', array( 'post' ) );

?>

<section <?php theme_block_attributes( $block, 'licitacoes-filter alignwide' ); ?>>
        ...
        <h1>
            <?php 
                ...
                if( get_field( 'title' ) ) {
                    ...
                    echo apply_filters( 'licitacoes_filter_title', $title );
                }
            ?>
        </h1>
    </header>
    ...
    <?php
        ...
        $filter_taxonomies = get_field( 'taxonomies' ) ? get_field( 'taxonomies' ) : apply_filters( 'licitacoes_filter_taxonomies', $post_type_taxonomies, $queried_object );
    ?>
    
    <?php if ( ! empty( $filter_taxonomies ) ) : ?>
        <form class="licitacoes-filter-filters">
            <div>
                <div class="sr-only">
                ...
                <a href="#<?php echo esc_attr( theme_block_id( $block ) ); ?>-licitacoes-filter-content-grid" class="sr-only ignore skip-filters"><?php esc_html_e( 'Skip filters', 'bulk' ); ?></a>
            </div>
            ...
            <?php if (is_array($filter_taxonomies)) : ?>
                <?php foreach( $filter_taxonomies as $filter_taxonomy ): ?>
                    ...
                    <div class="licitacoes-filter-filters-group" data-taxonomy="<?php echo $filter_taxonomy; ?>">
                        <div class="licitacoes-filter-filters-group-heading">
                            <button type="button" class="licitacoes-filter-filters-group-clear" title="<?php printf(esc_attr__( 'Clear All %s', 'bulk' ), $taxonomy_details->label); ?>" aria-label="<?php printf(esc_attr__( 'Clear All %s', 'bulk' ), $taxonomy_details->label); ?>" tabindex="-1" aria-hidden disabled></button>
                            <button type="button" class="licitacoes-filter-filters-group-title" aria-expanded="true" aria-haspopup="listbox" role="combobox">
                                <h2><?php echo $taxonomy_details->label; ?></h2>
                            </button>
                        </div>
                        <div class="licitacoes-filter-filters-group-options" aria-label="<?php echo $taxonomy_details->label; ?>">
                            <?php
                            ...
                            <div class="licitacoes-filter-filters-group-options-item">
                                <?php
            ...
            <div class="licitacoes-filter-filters-submit">
    ...
    <?php endif; ?>
    <div class="licitacoes-filter-content-wrapper">
		<div class="licitacoes-filter-content-loading">
			<?php theme_block_asset( 'img/loading.svg' ); ?>
		</div>
        <div class="licitacoes-filter-content-inner">
            <?php
                if ( ! $uses_custom_query ) {
                    global $wp_query;
			        $query = $wp_query;
            ...
            <?php if ( $query->have_posts() ) : ?>
                <div class="licitacoes-filter-content-grid" id="<?php echo esc_attr( theme_block_id( $block ) ); ?>-licitacoes-filter-content-grid" tabindex="0">
                    <?php while ( $query->have_posts() ) : ?>
                        <?php $query->the_post(); ?>
                        <?php get_template_part( 'template-parts/licitacoes', get_post_type() ); ?>
                    <?php endwhile; ?>
                </div>
                <div class="licitacoes-filter-content-pagination">
            ...
            <?php else : ?>
            <div class="licitacoes-filter-content-not-found" id="<?php echo esc_attr( theme_block_id( $block ) ); ?>-licitacoes-filter-content-grid" tabindex="0">
