...
<div <?php theme_block_attributes( $block, 'announcements-carousel' ); ?> data-color-scheme="<?php the_field( 'color_scheme' ); ?>" data-content-width="<?php the_field('content_width'); ?>">
	<?php if( have_rows( 'slides' ) ): ?>
	<div class="announcements-carousel-swiper swiper" data-autoplay="<?php the_field( 'enable_carousel_autoplay' ); ?>" data-autoplay-timeout="<?php the_field( 'carousel_slide_duration' ); ?>" data-start-on-next="<?php the_field( 'start_carousel_on_next_slide' ); ?>">
		<?php $slides = get_field( 'slides' ); ?>
		<?php if ( count( $slides ) > 1 ): ?>
            <?php if ( get_field( 'enable_carousel_pagination' ) ) : ?>
			<div class="announcements-carousel-pagination" role="presentation"></div>
			<?php endif; ?>
            <?php if ( get_field( 'enable_carousel_navigation' ) ) : ?>
			<div class="announcements-carousel-navigation">
				<button class="announcements-carousel-navigation-prev" aria-label="<?php esc_attr_e( 'Previous Slide', 'bulk' ); ?>" title="<?php esc_attr_e( 'Previous Slide', 'bulk' ); ?>">
					<?php theme_block_asset( 'img/nav-button-left.svg' ); ?>
				</button>
				<button class="announcements-carousel-navigation-next" aria-label="<?php esc_attr_e( 'Next Slide', 'bulk' ); ?>" title="<?php esc_attr_e( 'Next Slide', 'bulk' ); ?>">
		...
		<div class="swiper-wrapper">
			<?php while( have_rows( 'slides' ) ): ?>
				...
				<div class="swiper-slide">
					<div class="announcements-carousel-slide">
						<?php $button = get_sub_field( 'link' ); ?>
						<?php if ( 'image' === get_sub_field( 'media_type' ) ) : ?>
							<?php $image = get_sub_field( 'image' ); ?>
							<?php if ( ! empty( $image ) ) : ?>
							<div class="announcements-carousel-media">
								<div class="announcements-carousel-image">
									<?php if ( ! empty( $button ) ) : ?>
						...
						<?php elseif ( in_array( get_sub_field( 'media_type' ), array( 'youtube', 'vimeo', 'video' ) ) ) : ?>
							<div class="announcements-carousel-media">
								<div class="announcements-carousel-video-player">
